...
- 登录后使用的浏览器软电话、短信收发、通话记录、未接来电通知和可按线路启用的本地语音留言；录音只保存在网关，不随通知或支持包发送。默认不开放独立 SIP 客户端接入；本机打开 `allow_external_sip` 后可登记话机或中继。
...
- Telegram 默认仅用于单向推送来电、短信和设备状态通知；本机打开 `allow_telegram_commands` 后可用 /status /sms /call /hangup 操作本机线路。
...
> **合规警告：** 本软件仅供号码实名持有人在运营商明确允许的范围内自用。严禁用于诈骗、群呼、营销骚扰、验证码接收、号码或线路出租、代拨转接、隐藏实际控制地点，或向第三人提供电信服务。使用者必须遵守所在地法律、电话实名制和运营商协议；本项目不构成任何电信业务许可或运营商授权。默认最多保存和运行 **5 条 SIM 线路**，不提供独立 SIP 账号或 Telegram 远程拨号、发短信及挂断功能。本机可通过 `$MDD_DATA/local.yaml` 或系统设置放宽这四项（线路上限天花板 32）；说明见 [docs/LOCAL.md](docs/LOCAL.md)。技术限制不代表某种使用方式当然合法。
